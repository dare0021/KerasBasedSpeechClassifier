...
# TODO: find out why the MNIST data is (10000, 1, 28, 28) instead of (10000, 28*28)
# 		to show there is only one data in the sequence? (there is no sequence, just the image)
# 		also, shuffle the training data (but not the test)
...
# variant number of images, single number per pixel, y size, x size
# X_train = X_train.reshape(X_train.shape[0], 1, img_rows, img_cols)
# X_test = X_test.reshape(X_test.shape[0], 1, img_rows, img_cols)
# X_train = X_train.astype('float32')
# X_test = X_test.astype('float32')
# # pixel intensity stored as bytes. /=255 to move the range from [0, 255] to [0, 1]
# X_train /= 255
# X_test /= 255
...
# model.add(Convolution2D(nb_filters, nb_conv*8, nb_conv*8,
#                         border_mode='valid',
#                         input_shape=(1, img_rows, img_cols)))
# model.add(Activation('relu'))
# model.add(Convolution2D(nb_filters, nb_conv*4, nb_conv*4))
# model.add(Activation('relu'))
